...
def gerenciar_evento(request):
    if request.method == "GET":
        ...
        eventos = Evento.objects.filter(criador=request.user)      #filtro de busca do banco , se não usar o All()
        if nome:
...
def participantes_evento(request, id):
    ...
    if request.method == "GET":
        participantes = evento.participantes.all()[::3] ## mostrar apenas 3 
